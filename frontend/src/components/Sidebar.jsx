...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
    ...
    return (
        // max-sm:absolute 屏幕小于等于sm时，绝对定位
        ...
        max-sm:absolute top-0 bottom-0 z-20 ${
            // translate-x-0 始终显示
            // max-sm:-translate-x-full 屏幕小于等于sm时，隐藏(向左移动整个宽度)
            sidebarOpen ? 'translate-x-0' : 'max-sm:-translate-x-full'
        } transition-all duration-300 ease-in-out`}>
            <div className='w-full'>
                {/* onClick 作用：点击事件,跳转到首页 */}
                ...
                bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-800 active:scale-95 transition
